...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze a smoke experiment assignment bundle.")
...
def main() -> None:
    ...
    run_name = config.get("run_name", "experiment_analysis_smoke")
    run_dir = create_run_dir(run_name, base_dir=config["artifacts"]["base_dir"])

    summary, readout = analyze_experiment(config)
    write_yaml(run_dir / "config.yaml", config)
    write_json(run_dir / "summary.json", summary)
    write_json(run_dir / "readout.json", readout)
    print(f"Wrote experiment analysis bundle to {run_dir}")
